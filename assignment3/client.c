...
void printArray (DB_T d)
{
	for (int i = 0; i < d->numItems; i++) {
		printf("Customer %d: Name: %s, ID: %s, # of purchase: %d\n", i+1, (d->pArray)[i].name, (d->pArray)[i].id, (d->pArray)[i].purchase);
	}
	printf("\n");
	return;
}*/
...
struct DB {
  struct UserInfo **idTable;   // pointer to the array
...
void printArray (DB_T d) 
{
	...
	for (int i = 0; i < d->curArrSize; i++) {
		if ((d->idTable)[i] != NULL) printf("Hash value: %d\n", i);
    	for (struct UserInfo *p = (d->idTable)[i]; p != NULL; p = p->idnext) {
			k++;
			char *nextid = ((p->idnext) == NULL) ? NULL : (p->idnext)->id;
			char *nextname = ((p->namenext) == NULL) ? NULL : (p->namenext)->name;
			printf("%d) Name: %s, ID: %s, # of purchase: %d, next id: %s, next name: %s\n",
					k, p->name, p->id, p->purchase, nextid, nextname);
    	}
	}
	printf("\n");
	return;
