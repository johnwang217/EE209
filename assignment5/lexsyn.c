...
alias_lexLine(const char *pcLine, DynArray_T oTokens){
  enum aliasState {STATE_START, STATE_IN_WORD, STATE_IN_EQUAL};
  enum aliasState aState = STATE_START;
    ...
    switch (eState) {
      case STATE_START:
        if ((c == '\n') || (c == '\0'))
          ...
          if (createToken(oTokens, TOKEN_BG, NULL) == FALSE)
            return LEX_NOMEM;
